...
| --- | --- | --- |
| Node.js | **22.18.0** | `better-sqlite3` needs a prebuilt binary; Node 20/22 LTS have them |
| npm | 11.x (any recent) | |
...
| --- | --- | --- | --- |
| `GROQ_API_KEY` | no | — | Optional generation provider/fallback; select explicitly with `KIVI_CHAT_PROVIDER=groq` |
| `GROQ_API_KEYS` | no | — | Extra Groq keys, comma separated, rotated on rate limits |
| `KIVI_CHAT_PROVIDER` | no | `gemini` | who answers Hey Kivi: `auto` (Groq if keyed), `groq`, `gemini` |
...
| `KIVI_CHAT_MODEL` | no | `gemini-3.5-flash-lite` | Hey Kivi's model. Override only with a model available to your API account |
...
| `KIVI_EMBED_MODEL` | no | `gemini-embedding-2` | retrieval embeddings. Provider spaces are searched separately. Reset and reseed after changing the model or dimension |
...
> **Quota notes.** Provider limits depend on your account and model. Reduce ingestion
> concurrency or set evaluation pacing if calls are rate-limited. `auto` embeddings fall
> back to the local embedder; `gemini` requires the API. Provider counts are visible in
> `npm run db:summary`; `npm run reindex` rebuilds embeddings when quota is available.
...
the end. Runtime and estimated cost depend on model choices and API quotas; use the printed
ingestion statistics and the generated evaluation report for the measured run. `npm run setup` does both steps in one command.
...
6. **"Do I have any doctor appointments coming up?"** — it *is* in the corpus, and Kivi should
   answer only from the original dictation, never a memory, if the record supports an
   appointment. Check `/history`, search "neurologist", and verify that no memory was learned.
   Ask for a staging password separately: credentials must never be repeated.
...
Runs all 34 question cases plus 9 memory-state checks against the live database and writes:
...
This makes real model calls; timing and estimated cost are recorded in the report.
Relative-time questions use the day after the latest source dictation as their reference
date, printed at startup and saved in the report. To run one group only:
...
`abstention`, `privacy`, `clarification`, `episodic`, `control`, `honesty`.
`--only` also accepts a case ID or a comma-separated list of groups and IDs.
...
35 assertions over the parts that must not fail quietly and need no model: the import
...
whether the problem is the data or the pipeline. These checks also cover citation
parsing, date shifting and SQLite retrieval over isolated fixtures.
...
- `--keep-dates` — **by default the importer shifts timestamps by whole UTC days so the newest
  dictation lands yesterday**, preserving clock times and all intervals; this keeps
  phrases like "yesterday at 5pm" meaningful whenever the demo is run. Pass this flag to
  import the timestamps exactly as supplied. Unspecified clock times in Hey Kivi are UTC.

Ingesting into a database that already has data is supported and additive: re-running the
same IDs does not duplicate them. Previously processed IDs skip memory extraction; reset
and re-import if transcript content has changed and you need memory rebuilt.
